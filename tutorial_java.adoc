...
The table above shows how the HTTP method becomes a suffix on the URL path to create the CloudI service name pattern suffix.
...
A CloudI service name pattern is used with a subscribe CloudI API function call to declare a service process will accept service requests with a matching service name.
...
All services are structured for a 3 part sequence: initialization, requests, and termination.
