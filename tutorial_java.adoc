...
The book recommendation service initialization is shown below (from link:https://github.com/CloudI/cloudi_tutorial_java/blob/v1.5.4/src/main/java/org/cloudi/examples/tutorial/Service.java#L90-L140[Service.java:90-140^]):
...
An example of a service request handling function is below (using the "itemRefresh" function from link:https://github.com/CloudI/cloudi_tutorial_java/blob/v1.5.4/src/main/java/org/cloudi/examples/tutorial/Service.java#L163-L209[Service.java:163-209^]):
...
[[{prefix, "/tutorial/java/service/"},
  ...
  {args, "-Dfile.encoding=UTF-8 "
         ...
         "-ea:org.cloudi... "
         "-Xms3g -Xmx3g "
         "-jar $PWD/target/cloudi_tutorial_java-1.5.4-jar-with-dependencies.jar "
...
The table above shows the service subscriptions that occur during service initialization (in link:https://github.com/CloudI/cloudi_tutorial_java/blob/v1.5.4/src/main/java/org/cloudi/examples/tutorial/Service.java#L90-L140[Service.java:90-140^]).
...
The Javascript interface relies on a WebSocket connection for any usage of the REST API as shown below (link:https://github.com/CloudI/cloudi_tutorial_java/blob/v1.5.4/html/tutorial/java/index.html#L542-L550[index.html:542-550^]):
...
The service request handler function "client" is shown below (from link:https://github.com/CloudI/cloudi_tutorial_java/blob/v1.5.4/src/main/java/org/cloudi/examples/tutorial/Service.java#L429-L552[Service.java:429-552^]):
...
For scheduling an interrupt of the `poll` function the timeout parameter is used as shown below (from link:https://github.com/CloudI/cloudi_tutorial_java/blob/v1.5.4/src/main/java/org/cloudi/examples/tutorial/Service.java#L141-L146[Service.java:141-146^]):
...
Each interrupt of the `poll` function allows execution to check if it can run a chunk of function objects in the `ServiceIdle` class as shown below (from link:https://github.com/CloudI/cloudi_tutorial_java/blob/v1.5.4/src/main/java/org/cloudi/examples/tutorial/ServiceIdle.java#L9-L73[ServiceIdle.java:9-73^]):
